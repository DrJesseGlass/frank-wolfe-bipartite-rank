...
a count-weighted univariate loss, whose weights for the pairwise hinge are
computable in $O(N\log N)$ without forming pairs. This geometry unifies a family of known efficiency results and
...
% ---------------------------------------------------------------------------
\input{dual_coefficient_mappings}
% ---------------------------------------------------------------------------
...
$c_i = \sum_j \sigma(s_j - s_i)$; unlike the hard counts, evaluating the
soft counts exactly costs $O(n^+ n^-)$ (they can be approximated in
near-linear time, e.g.\ by binning the opposite-class scores, but we do not
rely on this).
\end{proposition}
